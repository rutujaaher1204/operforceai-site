...
//   const [dropdown, setDropdown] = useState(false); // product dropdown state
//   const timer = useRef<number | null>(null);
...
//   const hideMenu = () => {
//     timer.current = window.setTimeout(() => setDropdown(false), 200);
//   };
...
//             {dropdown && (
//               <div
//                 className="fixed left-1/2 -translate-x-1/2 mt-8 w-[1000px] min-h-[300px] 
//                            bg-white/10 backdrop-blur-lg border border-white/20 
//                            rounded-2xl p-6 shadow-lg flex justify-between items-center z-50"
//                 onMouseEnter={showMenu}
//                 onMouseLeave={hideMenu}
//               >
//                 <Link
...
//           <Link to="/careers" className="text-black hover:text-sky-300">
//             Careers
//           </Link>
//           <Link to="/support" className="text-black hover:text-sky-300">
//             Support
//           </Link>
//           <Link to="/about" className="text-black hover:text-sky-300">
//             About Us
//           </Link>
//         </nav>
...
export default function Navbar() {
  const [open, setOpen] = useState(false); // drawer state
  const [dropdown, setDropdown] = useState(false); // desktop products dropdown
  const [mobileProducts, setMobileProducts] = useState(false); // mobile expand products
  const timer = useRef<number | null>(null);
  ...
  return (
    <header className="absolute top-0 left-0 w-full z-50 bg-transparent">
      ...
      >
        ...
        {/* Drawer Links */}
        <nav className="flex flex-col items-center gap-4 p-6 text-lg">
          <Link to="/" onClick={() => setOpen(false)} className="hover:text-sky-400">
            Home
          </Link>

          <button
            onClick={() => setMobileProducts(!mobileProducts)}
            className="flex flex-col items-center justify-between w-full hover:text-sky-400"
          >
            <span>Products</span>
          </button>
          {mobileProducts && (
            <div className="ml-4 flex flex-col gap-3 text-base text-center text-gray-700">
              <Link to="/products/all" onClick={() => setOpen(false)}>All Products</Link>
              <Link to="/products/platform" onClick={() => setOpen(false)}>Enterprise Platform</Link>
              <Link to="/products/apps" onClick={() => setOpen(false)}>Enterprise Apps</Link>
              <Link to="/products/consulting" onClick={() => setOpen(false)}>Business Consulting</Link>
              <Link to="/products/mobile" onClick={() => setOpen(false)}>Mobile Apps</Link>
            </div>
          )}

          <Link to="/careers" onClick={() => setOpen(false)} className="hover:text-sky-400">
            Careers
          </Link>
          <Link to="/support" onClick={() => setOpen(false)} className="hover:text-sky-400">
            Support
          </Link>
          <Link to="/about" onClick={() => setOpen(false)} className="hover:text-sky-400">
            About Us
          </Link>
        </nav>
      </div>
